...
void addHeap(DynArr *heap, TYPE node)
{
	...
	if(pidx >= 0) parentVal = heap->data[pidx];

	while(pidx >= 0 && compare(node, parentVal) == -1){
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
	...
	while(1){
		...
		if(compare(heap->data[l], heap->data[swpIdx]) == -1){
			swpIdx = l;
		}
		if(r <= max && compare(heap->data[r], heap->data[swpIdx]) == -1){
			swpIdx = r;
		}
		
		swapDynArr(heap, pos, swpIdx);
		if(pos == swpIdx) return; /*nothing was switched*/
		pos = swpIdx;
	}
	/*
	if(r <= max){
		...
		if(compare(heap->data[l], heap->data[r]) == -1)
			swpIdx = l;
		else
			swpIdx = r;

		if(compare(heap->data[swpIdx], heap->data[pos]) == -1){
			swapDynArr(heap, pos, swpIdx);
			_adjustHeap(heap, max, swpIdx);
		}
	}
	else if(l <= max){
		if(compare(heap->data[l], heap->data[pos]) == -1){
			swapDynArr(heap, pos, l);
...
void _buildHeap(DynArr *heap)
{
	...
	for(i = heap->size / 2; i >= 0; i--){
		_adjustHeap(heap, heap->size-1, i);
	}
}
